...
function buildWeeks(today, n = 3) {
    return new Array(n).fill([]).map((week, idx) => {
        return week.concat(Array.from(moment.range(moment(today).subtract(idx, 'weeks').startOf('isoWeek'),
                                      idx ? moment(today).subtract(idx, 'weeks').endOf('isoWeek').isoWeekday(5) : moment(today).subtract(idx, 'weeks')).by('days')));
    }).reverse();
}
